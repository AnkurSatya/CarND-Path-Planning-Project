...
static double dt = 2.0;//time duration for predicting a goal state. It decides the length of the path.
static double time_for_maneuver = 2.0;// time duration to cover the trajectory.
static double timestep = 0.02;
static double SEARCH_DISTANCE= 1000.0;//(in meters) Search distance for the vehicles in a lane. It decides whether to take a vehicle in consideration or not.
