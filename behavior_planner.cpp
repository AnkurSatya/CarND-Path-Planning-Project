...
FSM::FSM() 
{
	...
	// state_graph["KL"] = {"LCR", "LCL"};
	state_graph["PLCL"] = {"KL","LCL","PLCL"};
	state_graph["LCL"] = {"KL","PLCL","LCL"};//Added LCL so that the vehicle moves to the lane center based on the value of the cost function(distance from the lane's center), KL and PLCL would give the same result for this cost function.
	state_graph["PLCR"] = {"KL","LCR","PLCR"};
	state_graph["LCR"] = {"KL","PLCR","LCR"};//	
}
FSM::~FSM() {}
...
vector<double> Vehicle::get_kinematics(Vehicle &vehicle, int target_lane)
{
	...
	if(vehicle_ahead_id != -1)
	{
		...
		{
			new_velocity = (vehicle_ahead_kinematics[0] - s - BUFFER_DISTANCE - 0.5*s_dot_dot*dt*dt)/dt + vehicle_ahead_kinematics[1];// The acceleration term is added to negate the effect of the acceleration at the previous timestamp on the new velocity.
			new_velocity = min(min(MAX_ACCELERATION*dt + s_dot, new_velocity), MAX_SPEED);
			cout<<"Vehicle Ahead Velocity = "<<vehicle_ahead_kinematics[1]<<endl;
	...
	new_kinematics = {s, new_velocity, s_dot_dot, double(int(d/4)*4.0 + 2), d_dot, d_dot_dot}; //converting d to integer to avoid the final position of the ego vehicle from being outside a lane.
	return new_kinematics;
}

int FSM::find_goal_pose(Vehicle &vehicle, string start_state, string goal_state)
{
	...
	else if(goal_state.compare("LCL")== 0 ||  goal_state.compare("LCR") == 0)//Change lane.
	{
		...
		for(it; it != vehicle.predictions.end(); ++it)
		{
			...
			{
				if(fabs(vehicle.predictions.find(it->first)->second[0] - new_lane_kinematics[0]) <= BUFFER_DISTANCE)
				{
					...
					cout<<"    "<<"Distance from ego vehicle = "<<vehicle.predictions.find(it->first)->second[0] - new_lane_kinematics[0]<<endl;
					return 2;
				}
			}
		}		
...
vector<vector<double>> FSM::next_state(Vehicle &vehicle, string &current_state)
{
	...
	{	
		...
		int ret = find_goal_pose(vehicle, current_state, goal_state);
		if(ret == 1 || ret == 2)
		{
			costs[goal_state] = MAX_COST;
			cout<<"Total Cost = "<<MAX_COST<<endl;
...
vector<vector<vector<double>>> FSM::generate_multiple_goal_states(vector<vector<double>> original_state, int num_states, double std_dev_s, double std_dev_d)
{
	...
	while(i<num_states)
	{
		...
		i++;
		// cout<<goal_state[0]<<", "<<goal_state[3]<<endl;
	}
	return goal_states;
}
...
vector<vector<vector<double>>> FSM::best_state_waypoints(vector<vector<double>> original_goal_state,Vehicle &vehicle, generate_trajectory traj, vector<vector<vector<double>>> goal_states, vector<double> map_waypoints_s, vector<double> map_waypoints_x, vector<double> map_waypoints_y, int num_waypoints, int num_waypoints_from_previous_path, string goal_state_name)
{
	...
	for(int i = 0; i<goal_states.size(); i++)
	{
		...
		vector<vector<double>> coeffs = traj.generate_JMT(goal_state);
